...
static void rknand_init(struct rk_nand *rknand)
{
...
static void rknand_select_chip(struct mtd_info *mtd, int chipnr)
{
...
static void rknand_cmd_ctrl(struct mtd_info *mtd, int dat, unsigned int ctrl)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct rk_nand *rknand = to_rknand(chip->controller);
...
static void rknand_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct rk_nand *rknand = to_rknand(chip->controller);
	int offs = 0;
...
static void rknand_write_buf(struct mtd_info *mtd, const uint8_t *buf, int len)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct rk_nand *rknand = to_rknand(chip->controller);
	int offs = 0;
...
static uint8_t rknand_read_byte(struct mtd_info *mtd)
{
	uint8_t ret;

	rknand_read_buf(mtd, &ret, 1);

	return ret;
}

static int rknand_dev_ready(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct rk_nand *rknand = to_rknand(chip->controller);
...
static int rknand_hw_ecc_setup(struct mtd_info *mtd,
...
static void rknand_pio_xfer_start(struct rk_nand *rknand, u8 dir, u8 st_buf)
{
	u32 reg;
...
static int rknand_wait_pio_xfer_done(struct rk_nand *rknand)
{
...
static void rknand_read_extra_oob(struct mtd_info *mtd, u8 *oob)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	...
	rknand_read_buf(mtd, oob + offset, len);
}

static void rknand_write_extra_oob(struct mtd_info *mtd, u8 *oob)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct nand_ecc_ctrl *ecc = &chip->ecc;
	int offset = ((ecc->bytes + ecc->prepad) * ecc->steps);
	...
	rknand_write_buf(mtd, oob + offset, len);
}


static int rknand_hw_syndrome_pio_read_page(struct mtd_info *mtd,
					    ...
					    int oob_required, int page)
{
	...
	if (rknand->bootromblocks && (offset < (7 * mtd->erasesize)))
		rknand_hw_ecc_setup(mtd, ecc, NANDC_V6_BOOTROM_ECC);

	rknand_pio_xfer_start(rknand, NANDC_V6_READ, 0);
	...
	for (step = 0; step < ecc->steps; step++) {
		...
		ret = rknand_wait_pio_xfer_done(rknand);
		if (ret)
			return ret;
		...
		if ((step + 1) < ecc->steps)
			rknand_pio_xfer_start(rknand, NANDC_V6_READ,
					      		(step + 1) & 0x1);
	...
	rknand_read_extra_oob(mtd, chip->oob_poi);

	if (rknand->bootromblocks)
		rknand_hw_ecc_setup(mtd, ecc, rknand->ecc_strength);

	return max_bitflips;
}

static uint32_t rknand_make_bootrom_compat(struct mtd_info *mtd, int page,
					   const u8 *oob, bool bootromblocks)
{
...
static int rknand_hw_syndrome_pio_write_page(struct mtd_info *mtd,
					     struct nand_chip *chip,
				       	     const uint8_t *buf,
					     int oob_required,
					     int page)
{
	...
	index = rknand_make_bootrom_compat(mtd, page, oob,
	...
	for (step = 1; step <= ecc->steps; step++) {
		rknand_pio_xfer_start(rknand, NANDC_V6_WRITE, (step - 1) & 0x1);
	...
	rknand_write_extra_oob(mtd, chip->oob_poi);
...
static int rknand_ecc_max_strength(struct mtd_info *mtd,
				   struct nand_ecc_ctrl *ecc)
{
...
static bool rknand_strength_is_valid(int strength)
{
	uint32_t index;
...
static int rknand_hw_ecc_ctrl_init(struct mtd_info *mtd,
				   struct nand_ecc_ctrl *ecc)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct rk_nand *rknand = to_rknand(chip->controller);
	...
	if (rknand_strength_is_valid(ecc->strength))
		strength = ecc->strength;
	else
		strength = rknand_ecc_max_strength(mtd, ecc);

	rknand_hw_ecc_setup(mtd, ecc, strength);
...
static int rknand_ecc_init(struct mtd_info *mtd, struct nand_ecc_ctrl *ecc)
{
	int ret;

	switch (ecc->mode) {
	case NAND_ECC_HW_SYNDROME:
		ret = rknand_hw_ecc_ctrl_init(mtd, ecc);
		if (ret)
			return ret;
		ecc->read_page =  rknand_hw_syndrome_pio_read_page;
		ecc->write_page = rknand_hw_syndrome_pio_write_page;
		break;
...
static int rknand_chip_init(int node, struct rk_nand *rknand, int devnum)
{
	...
	chip->select_chip = rknand_select_chip;
	chip->cmd_ctrl = rknand_cmd_ctrl;
	chip->read_buf = rknand_read_buf;
	chip->write_buf = rknand_write_buf;
	chip->read_byte = rknand_read_byte;
	chip->dev_ready = rknand_dev_ready;
	...
	ret = rknand_ecc_init(mtd, &chip->ecc);
	if (ret) {
		debug("rknand_ecc_init failed: %d\n", ret);
		return ret;
	}
...
static int rknand_chips_init(int node, struct rk_nand *rknand)
{
	const void *blob = gd->fdt_blob;
	int nand_node;
	int ret, i = 0;
	...
	for (nand_node = fdt_first_subnode(blob, node); nand_node >= 0;
		...
		ret = rknand_chip_init(nand_node, rknand, i++);
		if (ret)
			return ret;
	}
...
void board_nand_init(void)
{
	...
	ret = rknand_chips_init(node, rknand);
	if (ret) {
